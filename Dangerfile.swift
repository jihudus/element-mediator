...
if let ticketNumberRegex = try? NSRegularExpression(pattern: "#\\d+") {
    ...
    if missingTicketNumber {
        warn("Some of the commits are missing ticket numbers. Please consider using them for better tracking.")
    }
}
...
let hasChangedViews = !editedFiles.filter { $0.lowercased().contains("/layout") }.isEmpty
if hasChangedViews {
